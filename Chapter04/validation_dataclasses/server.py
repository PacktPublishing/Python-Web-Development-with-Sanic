...
class ValidatorModel:
    def __post_init__(self):
        for fld in fields(self.__class__):
            existing = getattr(self, fld.name)
            hydrated = self._hydrate(fld.type, existing)

            if hydrated:
                setattr(self, fld.name, hydrated)
            elif type(existing) is not fld.type:
                setattr(self, fld.name, fld.type(existing))
